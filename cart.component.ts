...
export class CartComponent {
  ...
  ngOnInit() {
    const navigation = window.history.state.cartArray;
    this.users = navigation;
  ...
  pay() {
    ...
    this.users.forEach((user: any) => {
      totalAmount += parseInt(user.Amount);
    });
  ...
  createRazorpayInstance() {
    ...
    this.users.forEach((user: any) =>{
      this.http.put(`http://localhost:8000/api/update/${user._id}`, newData).subscribe(
        (response: any) => {
